...
CREATE PROCEDURE [dbo].[CreateHospitalAdminUser]
...
BEGIN
	...
	SET XACT_ABORT ON;
	BEGIN TRY		
		BEGIN TRANSACTION CreateHospitalAdminUser		
		SET @userid = -1;
		...
		begin
			...
			--HospitalAdminUser
			EXEC InsertHospitalAdmin 
			@userid, @userguid,@hospitalId
		...
		COMMIT TRANSACTION CreateHospitalAdminUser
	END TRY
	BEGIN CATCH
		...
		BEGIN
			PRINT
				...
				'Rolling back transaction CreateHospitalAdminUser.'
		...
		BEGIN
			PRINT
				...
				'Committing transaction CreateHospitalAdminUser.'
			COMMIT TRANSACTION CreateHospitalAdminUser;
		END;
	END CATCH; 
	RETURN @userid
